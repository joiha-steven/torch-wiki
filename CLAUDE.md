...
- **LCP — image fade-in (`FlashlightCard`):** the `.img-load { opacity:0 } → .is-loaded` fade is **skipped for `priority` images** (the first 4 cards). The fade only flips to visible on the JS `onLoad`, so gating the above-the-fold image behind it kept the LCP element invisible until hydration (~2.3s of LCP "render delay"). Priority cards paint straight from the server HTML; below-the-fold cards keep the fade. Priority images also get an explicit `fetchPriority="high"` — this Next build does **not** derive it from the `priority` prop (Lighthouse flagged the missing hint).
- **Browse JS (perf):** card detail/brand `<Link>`s use **`prefetch={false}`** — a 32+ card grid was eagerly prefetching every SSG detail route, pulling the react-markdown chunk (~44KB, unused on browse) + 32 RSC requests into the initial load. Detail pages are edge SSG so click-through stays fast. A modern **`browserslist`** in `package.json` (safari≥15.4 etc.) drops Next/SWC's legacy ES6+ transforms/polyfills. JS is Brotli-compressed at the edge automatically — no manual compression. Don't re-introduce `@vercel/speed-insights` (removed — billed, was disabled in dashboard).
