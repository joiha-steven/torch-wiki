      ...
      <div className="max-w-3xl mx-auto px-6 py-10 sm:py-14">
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-6 text-sm">
            {page > 1
              ? <Link href={`/data-log?page=${page - 1}`} className="text-brand-600 font-medium hover:text-brand-500">← Newer</Link>
              : <span className="text-ink-3/40">← Newer</span>}
            <span className="text-ink-3 text-xs">Page {page} of {totalPages}</span>
            {page < totalPages
              ? <Link href={`/data-log?page=${page + 1}`} className="text-brand-600 font-medium hover:text-brand-500">Older →</Link>
