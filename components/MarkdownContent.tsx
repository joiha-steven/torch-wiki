...
function VideoEmbed({ src }: { src: string }) {
  return (
    <span className="block my-4 rounded-xl overflow-hidden border border-line bg-black aspect-video">
      <iframe
        src={src}
...
const components: Components = {
  ...
  h1:     ({ children }) => <h1 className="text-lg font-bold text-ink mt-6 mb-2 first:mt-0">{children}</h1>,
  h2:     ({ children }) => <h2 className="text-base font-bold text-ink mt-5 mb-2 first:mt-0">{children}</h2>,
  h3:     ({ children }) => <h3 className="text-sm font-semibold text-ink mt-4 mb-1.5 first:mt-0">{children}</h3>,
  ...
  img:    ({ src, alt }) => (
    ...
    <img
      ...
      alt={alt ?? ''}
      loading="lazy"
      className="block my-4 max-w-full h-auto rounded-xl border border-line bg-panel"
    />
  ),
  code:   ({ children }) => (
    <code className="bg-slate-100 dark:bg-white/[0.05] text-ink-2 px-1.5 py-0.5 rounded text-[0.85em] font-mono">{children}</code>
  ),
  pre:    ({ children }) => (
    <pre className="bg-slate-50 dark:bg-white/[0.04] border border-line rounded-lg p-3 overflow-x-auto text-xs font-mono my-3">{children}</pre>
  ),
  hr:     () => <hr className="border-line my-5" />,
  blockquote: ({ children }) => (
    <blockquote className="border-l-2 border-brand-400 pl-4 text-ink-3 italic my-4">{children}</blockquote>
  ),
  table:  ({ children }) => (
  ...
  th:     ({ children }) => <th className="py-2 pr-4 font-semibold text-ink">{children}</th>,
  td:     ({ children }) => <td className="py-2 pr-4 border-t border-line align-top">{children}</td>,
}
