...
export default function BrandEditForm({ name, initial, onClose, onSaved }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto p-4 sm:p-8">
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-2xl my-auto shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div>
            <h2 className="font-semibold text-slate-900">Edit {name}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Brand origin (country)</label>
              <input className={input} value={country} onChange={e => setCountry(e.target.value)} placeholder="e.g. China" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Made in</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Headquarters</label>
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={onClose} className="px-4 h-10 rounded-full text-sm text-slate-500 hover:text-slate-800">Cancel</button>
            <button
              type="submit"
              disabled={saving}
